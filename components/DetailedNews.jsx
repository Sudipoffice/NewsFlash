...
const DetailedNews = () => {
    ...
    const { source, ...newsDetails } = location.state ;
  ...
  return (
    ...
    <div id="detailedNews" className='flex flex-col md:flex-row min-h-screen w-screen justify-center items-start p-4 py-2 md:p-20 gap-2 md:gap-12'>
     {/*  DetailedNews */}
        <section className='flex flex-col  w-full md:w-4/6 gap-1 md:gap-4 py-12 text-start'>
            <img className='aspect-5/3' src={source==="World"? news.image : news?.multimedia[0]?.url} alt={news.title}/>
            <p className='font-semibold text-xs md:text-lg text-gray-600' style={{fontFamily: "Antonio"}}>{source==="World"? news.publish_date.slice(0,10): news.published_date.slice(0,10)}</p>
            <h1 className='text-xl md:text-5xl text-bold uppercase' style={{fontFamily: "Anton"}}>{news.title}</h1>
            <p className='text-sm md:text-lg w-3/4'>{source==="World"? news.text : news.abstract}</p>
            <p className='uppercase font-semibold text-[0.6rem] md:text-xl text-gray-600' style={{fontFamily: "Antonio"}}>{source==="World"? news.authors : news.byline}</p>
        </section>
        {/* Extra News */}
